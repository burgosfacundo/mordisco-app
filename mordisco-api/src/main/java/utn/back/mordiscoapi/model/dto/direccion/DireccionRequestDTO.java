...
public record DireccionRequestDTO(
    @Size(message = "La calle no puede contener mas de 50 caracteres", max = ValidationConstants.CALLE_MAX_LENGTH)
    @NotBlank(message = "La calle es obligatoria")
    @Schema(description = "Calle de la dirección", example = "Alvarado")
    String calle,

    @Size(message = "El numero no puede contener mas de 5 caracteres", max = ValidationConstants.NUMERO_MAX_LENGTH)
    ...
    @Size(message = "El piso no puede contener mas de 20 caracteres", max = ValidationConstants.PISO_MAX_LENGTH)
    @Schema(description = "Piso de la dirección", example = "12")
    String piso,

    @Size(message = "El departamento no puede contener mas de 20 caracteres", max = ValidationConstants.DEPTO_MAX_LENGTH)
    @Schema(description = "Departamento de la dirección", example = "C")
    String depto,

    @Size(message = "El código postal no puede contener mas de 10 caracteres", max = ValidationConstants.CODIGO_POSTAL_MAX_LENGTH)
    @NotBlank(message = "El código postal es obligatorio")
    @Schema(description = "Código postal de la dirección", example = "7600")
    @Pattern(regexp = "^[0-9A-Za-z\\- ]{1,12}$")
    String codigoPostal,

    @Size(message = "La referencia no puede contener mas de 255 caracteres", max = ValidationConstants.REFERENCIAS_MAX_LENGTH)
    @Schema(description = "Referencia de la dirección", example = "Edificio al lado de porton rojo")
    String referencias,

    @Size(message = "El alias no puede contener mas de 50 caracteres", max = ValidationConstants.ALIAS_MAX_LENGTH)
